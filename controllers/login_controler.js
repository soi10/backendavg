...
exports.login = async (req, res, next) => {
  ...
  if (resp.Envelope.Body.LoginResponse.LoginResult.ResponseMsg === "Success") {
    ...
    const dataUser = {
      ...
      CostCenterName:
        resp2.Envelope.Body.GetEmployeeInfoByEmployeeIdResponse
          ...
          CostCenterCode:
        resp2.Envelope.Body.GetEmployeeInfoByEmployeeIdResponse
          .GetEmployeeInfoByEmployeeIdResult.ResultObject.CostCenterCode,
          Peacode:
        resp2.Envelope.Body.GetEmployeeInfoByEmployeeIdResponse
    ...
    res.status(200).json({
      token: token,
      EmployeeId:dataUser.EmployeeId,
      FullName:dataUser.FullName,
      DepartmentShortName:dataUser.DepartmentShortName,
      DepartmentFullName:dataUser.DepartmentFullName,
      Position:dataUser.Position,
      BaCode:dataUser.BaCode,
      BaName:dataUser.BaName,
      CostCenterName:dataUser.CostCenterName,
      CostCenterCode:dataUser.CostCenterCode,
      Peacode:dataUser.Peacode,
      statuscode:"200"
    });
  } else {
    res.status(401).json({ message: 'Unauthorized', data: req.body });
  }
};