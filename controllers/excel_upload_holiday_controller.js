const db = require("../models/holiday")
const uploads = db;
...
const uploadExcelHoliday = async function (req, res, next) {
    try {
        ...
        for (i = 0; i < jsonData.length; i++) {
            let data = {
                dateHoliday: jsonData[i].date,
                nameHoliday: jsonData[i].detail,
                yearHoliday:jsonData[i].date.substring(0, 4)
            }
